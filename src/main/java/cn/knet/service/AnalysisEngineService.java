...
public class AnalysisEngineService {
    ...
    public DbResult updateForSelect(String type, String sql,int i) {
        ...
        try {
            ...
            String selectSql = getSelectSql(updateStatement,SpringTools.getJdbcTemplate(type));
    ...
    public DbResult alertAnalysisEngine(String type, String sql, String userId) {
        ...
        try {
            ...
            log.info(msg);
            logEngineService.logSava(new KnetSqlLog(sql, type, sqlType.name(), msg, userId),null);
     ...
     * @return
     */
    public DbResult updateForPLAnalysisEngine(String type, String[] sqls, String userId, String table) {
        long startTime = System.currentTimeMillis();
        String error = "";
        int i = 0;
        List<KnetSqlLogDetail> details=new ArrayList<>();
        for (String sql : sqls) {
            ...
            try {
                ...
                i += SpringTools.getJdbcTemplate(type).update(sql);
            } catch (Exception e) {
                error += "sql:"+sql + "更新出错（" + SqlParserTool.getSqlEcception(e) + ")";
                log.error("更新出错{}", error);
            }
        }
        long time = System.currentTimeMillis() - startTime;
        String msg = i + "条数据被成功更新到表" + table + (StringUtils.isNotBlank(error) ? ("," + error) : ",") + "执行时间" + time + "毫秒。";
        log.info(msg);
        logEngineService.logSava(new KnetSqlLog(Arrays.stream(sqls).collect(Collectors.joining(";")), type, SqlType.PIUPDATE.name(),msg, userId),
                details);
        return DbResult.success(msg, Arrays.stream(sqls)
    ...
    public DbResult insertAnalysisEngine(String type, String sql, String userId) {
        ...
        try {
            ...
            log.info(msg);
            logEngineService.logSava(new KnetSqlLog(sql, type, SqlType.INSERT.name(), msg, userId),null);
            return DbResult.success(msg, sql);
        } catch (Exception e) {
     ...
     * @return
     */
    public String getSelectSql(Update updateStatement,JdbcTemplate jdbcTemplate) {
